...
class simpleSeparator4(nn.Module):
    def __init__(self, num_spks, channels, block, block_channels):
        super(simpleSeparator4, self).__init__()
        self.num_spks = num_spks 
        self.channels = channels #this is dependent on the dac model
        self.block = copy.deepcopy(block) #this should be a seq2seq model with identical input and output sizes
        self.enh_block = copy.deepcopy(block)
    ...
    def forward(self,x):
        ...
        #Enhancement (with shared weights)
        x = self.ch_down(x)
        #[B,N,L]
        x = x.permute(0,2,1)
        #[B,L,N]
        x_b = self.enh_block(x)
        #[B,L,N]
        x_b = x_b.permute(0,2,1)
        #[B,N,L]
        x = self.ch_up(x_b)
...
class simpleSeparator3(nn.Module):
    def __init__(self, num_spks, channels, block, block_channels):
        super(simpleSeparator3, self).__init__()
        self.num_spks = num_spks 
        self.channels = channels #this is dependent on the dac model
        self.block = block #this should be a seq2seq model with identical input and output sizes
        self.ch_down = nn.Conv1d(channels, block_channels,1,bias=False)
