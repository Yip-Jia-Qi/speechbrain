...
class Separation(sb.Brain):
    ...
    def compute_objectives(self, predictions, targets, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            C, B, N, L = targ_w.shape
            targ_w_cfm_batch = targ_w.view(B*C,N,L).permute(0,2,1)
            
            cfm_loss = self.hparams.vbCFM(targ_w_cfm_batch)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        with torch.no_grad():
            predictions, targets, targets_transmitted, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes  = self.compute_forward(mixture, targets, stage, use_cfm=use_cfm)
