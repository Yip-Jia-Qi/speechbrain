...
class Separation(sb.Brain):
    def compute_forward(self, mix, targets, stage, noise=None, use_cfm=False):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if use_cfm:
                assert mix_s.shape[1] == 1
                mix_s = torch.cat(
                    [
                        self.hparams.vbCFM.sample(cond  = mix_s[i].permute(0,2,1)).permute(0,2,1).unsqueeze(0)
                        for i in range(self.hparams.num_spks)
                    ],
                    dim=0,
                )
    ...
    def compute_objectives(self, predictions, targets, other, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            C, B, N, L = other["targ_q"].shape
            targ_q_cfm_batch = other["targ_q"].view(B*C,N,L).permute(0,2,1)
            
            mix_s_batch = other["mix_s"].view(B*C,N,L).permute(0,2,1)

            cfm_loss = self.hparams.vbCFM(targ_q_cfm_batch, cond = mix_s_batch)
            
            loss = emb_loss + cfm_loss
        ...
        if stage == sb.Stage.TEST and self.hparams.save_audio:
            if hasattr(self.hparams, "n_audio_to_save"):
                if self.hparams.n_audio_to_save > 0:
                    self.save_audio(snt_id[0], mixture, targets, predictions, use_cfm=use_cfm)
    ...
    def save_audio(self, snt_id, mixture, targets, predictions, use_cfm=False):
...
if __name__ == "__main__":
    ...
    separator.evaluate(test_data, min_key="si-snr")    
    separator.plot_metric_histograms(separator.hparams.epoch_counter.current, separator.use_cfm)
