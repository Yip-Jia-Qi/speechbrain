...
class Separation(sb.Brain):
    ...
    def evaluate_batch(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            loss = self.compute_objectives(predictions, targets_transmitted, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes, stage) #Sending targets transmitted here for cSISDR monitoring
...
if __name__ == "__main__":
    ...
    import copy
    class codecformerEL(nn.Module):
        def __init__(self):
            super(codecformerEL, self).__init__()

            # self.dacmodel = separator.hparams.dacmodel
            # self.sepmodel = separator.hparams.sepmodel


            from speechbrain.lobes.models.codecformer3 import DACWrapper
            self.dacmodel = DACWrapper(
            ...
            from speechbrain.lobes.models.codecformer3 import simpleSeparator2
            self.sepmodel = simpleSeparator2(
                num_spks = 2,
                channels = 1024,
                block =  sb.lobes.models.dual_path.SBTransformerBlock(
                    num_layers = 16,  #16
                    d_model = 256,
                    nhead = 8,  #1/8
                    d_ffn = 1024,             #2048?
        ...
        def forward(self, x):
            ...
            #Send the dac model to device
            self.dacmodel.model.to(x.device)
            self.dacmodel.dac_sampler.to(x.device)
            self.dacmodel.org_sampler.to(x.device)
            self.sepmodel.to(x.device)

            mix_w, mix_length = self.dacmodel.get_encoded_features(x.unsqueeze(1))
        
            est_mask = self.sepmodel(mix_w)
    ...
    print(params)
    # macs, params = profile(model, inputs=(x, ), 
    #                     custom_ops={separator: count_your_model})
