...
class Separation(sb.Brain):
    def compute_forward(self, mix, targets, stage, noise=None):
        ...
        est_mix_1 = self.hparams.dacmodel.get_decoded_signal(targ_q[0]+targ_q[1],mix_length).unsqueeze(0)
        est_mix_2 = torch.sum(
                torch.cat(
                    [
                        self.hparams.dacmodel.get_decoded_signal(targ_q[i],mix_length).unsqueeze(0)
                        for i in range(self.hparams.num_spks)
                    ],
                    dim=0,
                ),
                dim=0,
                keepdim=True  
            )/self.hparams.num_spks

        est_mix = torch.cat([est_mix_1,est_mix_2],dim=0)
        est_mix = est_mix.squeeze(2).permute(1,2,0)
        
        return est_mix, targets, targets_transmitted, mix_s, targ_w, mix_sq, targ_q, mix_sq_codes, targ_q_codes
        ...
        with open(save_file, "w") as results_csv:
            ...
            with tqdm(test_loader, dynamic_ncols=True) as t:
                for i, batch in enumerate(t):
                    ...
                    # print(self.hparams.sample_rate)
