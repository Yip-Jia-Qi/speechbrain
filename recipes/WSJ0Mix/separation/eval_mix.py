...
class Separation(sb.Brain):
    def compute_forward(self, mix, targets, stage, noise=None):
        ...
        est_mix_1 = self.hparams.dacmodel.get_decoded_signal(targ_w[0]+targ_w[1],mix_length).unsqueeze(0)
        est_mix_2 = torch.sum(
                torch.cat(
                    [
                        self.hparams.dacmodel.get_decoded_signal(targ_w[i],mix_length).unsqueeze(0)
                        for i in range(self.hparams.num_spks)
                    ],
                    dim=0,
                ),
