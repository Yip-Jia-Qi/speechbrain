    ...
    def compute_forward(self, mix, targets, stage, noise=None, use_cfm=False):
        ...
        if stage == sb.Stage.TEST:
            ...
            if use_cfm:
                ...
                mix_s = torch.cat(
                    [
                        self.hparams.vbCFM.sample(cond  = other["mix_s_norm"][i].permute(0,2,1), steps = 32).permute(0,2,1).unsqueeze(0)
                        for i in range(self.hparams.num_spks)
                    ],
                    dim=0,
                )
        ...
        if stage != sb.Stage.TEST:            
            ...
            for p in perms:
                ...
                for i, pos in enumerate(p):                    
                    cfm_loss += self.hparams.vbCFM(other["targ_q_norm"][i].permute(0,2,1), cond = other["mix_s_norm"][pos].permute(0,2,1))
