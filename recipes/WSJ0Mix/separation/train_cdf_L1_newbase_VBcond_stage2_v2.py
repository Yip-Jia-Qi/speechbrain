...
from speechbrain.nnet.losses import PitWrapper, get_perceptual_with_si_snr_pit
...
class Separation(sb.Brain):
    def compute_forward(self, mix, targets, stage, noise=None, use_cfm=False):
        ...
        with torch.no_grad():
            ...
            # overwrite embeddings with quantiziation if quantization before separation is needed
        ...
        if stage == sb.Stage.TEST:
            with torch.no_grad():
                ...
                if use_cfm:
                    ...
                    mix_s = torch.cat(
                        [
                            self.hparams.vbCFM.sample(cond  = other["mix_s"][i].permute(0,2,1), steps = 32).permute(0,2,1).unsqueeze(0)
                            for i in range(self.hparams.num_spks)
                        ],
                        dim=0,
                    )
    ...
    def compute_objectives(self, predictions, targets, other, stage):
        ...
        if stage != sb.Stage.TEST:            
            ...
            _, perms = self.hparams.L1_loss(other["mix_s"].permute(1,2,3,0),other["targ_q"].permute(1,2,3,0), getPerms = True)
            ...
            for p in perms:
                ...
                for i, pos in enumerate(p):
                    # Duplicate the input
                    duplicate_factor = 2  # Increase this to increase batch size
                    targ_q_expanded = other["targ_q"][i].repeat(duplicate_factor, 1, 1)
                    mix_s_expanded = other["mix_s"][pos].repeat(duplicate_factor, 1, 1)

                    # Calculate loss on expanded batch
                    expanded_loss = self.hparams.vbCFM(targ_q_expanded.permute(0,2,1), cond=mix_s_expanded.permute(0,2,1))

                    # Scale back the loss
                    cfm_loss += expanded_loss / duplicate_factor

                batch_cfm_loss += cfm_loss/len(p)
            
            batch_cfm_loss = batch_cfm_loss/len(perms)
        ...
        with self.no_sync(not should_step):
            if self.use_amp:
                with torch.autocast(
                    ...
                    loss = self.compute_objectives(predictions, targets, other, sb.Stage.TRAIN)
    ...
    def save_results(self, test_data, use_cfm=False):
        ...
        csv_columns = ["snt_id", "sdr", "sdr_i", "si-snr", "si-snr_i", "C_sdr", "C_sdr_i", "C_si-snr", "C_si-snr_i", "pesq", "stoi", "ovrl", "sig", "bak", "p808"]
        ...
        with open(save_file, "w") as results_csv:
            ...
            with tqdm(test_loader, dynamic_ncols=True) as t:
                for i, batch in enumerate(t):
                    ...
                    Csisnr = self.compute_objectives(predictions, targets_transmitted, other, sb.Stage.TEST)
                    ...
                    sisnr_baseline = self.compute_objectives(mixture_signal, targets, other, sb.Stage.TEST)
                    Csisnr_baseline = self.compute_objectives(mixture_signal, targets_transmitted, other, sb.Stage.TEST)
                    ...
                    all_ovrl.append(ovrl_score)
                    all_sig.append(sig_score)
                    all_bak.append(bak_score)
                    all_p808.append(p808_score)
                ...
                row = {
                    ...
                    "pesq": np.array(all_pesq).mean(),
                    "stoi": np.array(all_stoi).mean(),
                    "ovrl": np.array(all_ovrl).mean(),
                    "sig": np.array(all_sig).mean(),
                    "bak": np.array(all_bak).mean(),
        ...
        logger.info("Mean stoi is {}".format(np.array(all_stoi).mean()))
        logger.info("Mean ovrl is {}".format(np.array(all_ovrl).mean()))
        logger.info("Mean sig is {}".format(np.array(all_sig).mean()))
    ...
    def plot_metric_histograms(self, epoch, use_cfm):
        ...
        # List of metrics to plot (excluding 'snt_id')
        metrics = [col for col in df.columns if col != 'snt_id']
        ...
        for metric in metrics:
            ...
            # Save the plot
            plt.savefig(os.path.join(save_path, f'{metric}_histogram.png'))
            plt.close()
        
        logger.info(f"Metric histograms saved in {save_path}")
