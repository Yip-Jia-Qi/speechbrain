...
#         )
    ...
    def evaluate_batch(self, batch, stage):
        ...
        if stage == sb.Stage.TEST and self.hparams.save_audio:
            ...
            snt_id = [self.hparams.evaluate_counter]
            self.hparams.evaluate_counter += 1
            if hasattr(self.hparams, "n_audio_to_save"):
                if self.hparams.n_audio_to_save > 0:
                    self.save_audio(snt_id[0], mixture, targets, predictions)
                    self.hparams.n_audio_to_save += -1
            elif hasattr(self.hparams, "audio_id_to_save"):
                if snt_id[0] in self.hparams.audio_id_to_save:
                    self.save_audio(snt_id[0], mixture, targets, predictions)
            else:
                self.save_audio(snt_id[0], mixture, targets, predictions)
        ...
        with open(save_file, "w") as results_csv:
            ...
            with tqdm(test_loader, dynamic_ncols=True) as t:
                for i, batch in enumerate(t):
                    ...
                    both_signals = [target, interferance]
                    both_signals = torch.cat(
                        [both_signals[i][0].unsqueeze(-1) for i in range(self.hparams.num_spks)], dim=-1,
                    ).unsqueeze(0)
                    
                    mixture_signal = mixture_signal.to(targets.device)
                    both_signals = both_signals.to(targets.device)
                    # print(both_signals.shape)
                    # print(mixture_signal.shape)
                    # raise Exception
                    from speechbrain.nnet.losses import get_si_snr_with_pitwrapper
                    
                    sisnr_baseline = get_si_snr_with_pitwrapper(mixture_signal, both_signals)
                    
                    # sisnr_baseline = self.compute_objectives(
                    #     mixture_signal, both_signals, spk_pred, batch, sb.Stage.TEST
                    # )
                    ...
                    sdr_baseline, _, _, _ = bss_eval_sources(
                        both_signals[0].t().cpu().numpy(),
                        mixture_signal[0].t().detach().cpu().numpy(),
                    )

                    sdr_i = sdr.mean() - sdr_baseline.mean()
                    
                    # print(sdr_baseline.mean())
                    # raise Exception 
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.provides("itf")
    def audio_pipeline_s2(s2_wav):
        itf = sb.dataio.dataio.read_audio(s2_wav)
        return itf.cpu().detach().numpy().astype(np.float32) ##check if this is necessary
...
#     )
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["mix","aux","itf","aux_len","spk_idx", "ref"]
    )
